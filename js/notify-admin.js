/* ══════════════════════════════════════════════════════════
   ...
   왜 만들었나
     예전에는 알림 경로가 두 갈래였다.
       · 신청서 → app_config/admin_targets 의 ID 목록으로 발송
       · 1:1 상담 → OneSignal 태그(role=admin) 필터로 발송
     태그는 브라우저 데이터를 지우면 날아가고, 어느 쪽도 결과를 확인하지
     않아서 "보냈다"는 로그만 남고 실제로 갔는지 알 수 없었다.

   실제로 겪은 함정 세 가지 (같은 실수를 반복하지 않도록 적어둔다)
     ① url 과 web_url 을 함께 보내면 OneSignal이 요청을 통째로 거부한다.
        → 웹앱이므로 web_url 하나만 보낸다.
     ② 페이지마다 Firebase SDK 버전이 다를 수 있다(상담 화면은 10.7.1).
        다른 버전으로 만든 db를 넘기면 타입 검사에서 그대로 터지고,
        오류가 try에 먹혀 알림도 기록도 없이 조용히 사라진다.
        → 발송기가 자기 버전으로 직접 연결한다. db를 받지 않는다.
     ③ include_aliases 로 보내면 응답에 recipients 가 아예 없다.
        그걸 0명으로 읽으면 멀쩡히 나간 알림을 '실패'로 표시하게 된다.
        → id 가 오고 errors 가 없으면 성공으로 본다.

   하는 일
     · 발송 경로를 하나로 통일
     · 응답을 열어 성공·실패를 제대로 판정
     · 결과를 app_config/notify_log 에 최근 30건 기록
       (관리자 화면이 이걸 읽어 경고와 내역을 보여준다)
